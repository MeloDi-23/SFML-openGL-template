...
FLAG1 = -I$(SFML_PATH)\include -Iinclude -I$(GLM_PATH) -I$(GLEW_PATH)\include -I$(ASSIMP_PATH)\include -Wall -O0 -g
...
FLAG_DEBUG = -L$(SFML_PATH)\lib -lsfml-graphics-d -lsfml-window-d -lsfml-system-d -lopengl32 -L$(GLEW_PATH)\lib\Release\Win32 -lglew32 -L$(ASSIMP_PATH)\lib -llibassimp -g

all: release debug

release: release/Main.exe src/gl.o src/model.o move
release/Main.exe: src/Main.o
	$(CXX) $^ -o $@ $(FLAG_RELEASE)
	copy $(SFML_PATH)\bin\*.dll release\ /Y
	copy $(GLEW_PATH)\bin\Release\Win32\*.dll release /Y
	copy $(ASSIMP_PATH)\bin\*.dll release /Y

debug: debug/Main.exe
debug/Main.exe: src/Main.o src/gl.o src/model.o
	$(CXX) $^ -o $@ $(FLAG_DEBUG)
	copy $(SFML_PATH)\bin\*.dll debug\ /Y
	copy $(GLEW_PATH)\bin\Release\Win32\*.dll debug /Y
	copy $(ASSIMP_PATH)\bin\*.dll debug /Y
